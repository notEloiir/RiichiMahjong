...
class Round:
    def __init__(self, competitors: list[Player], scores, non_repeat_round_no, match_type,
                 device, gui=None, round_replay=None, collect_data=False):
        ...
        self.gui = gui
        self.board = None
        self.reply_rounds = round_replay
        self.is_reply = (round_replay is not None)
        self.collect_data = collect_data
        self.collected_data: list[DataPoint] = []

        if collect_data and round_replay is None:
        ...
        self.open_melds_tile_ids = [[] for _ in range(4)]
        ...
        self.riichi_status = [RiichiStatus.DEFAULT for _ in range(4)]
        self.furiten_status = [FuritenStatus.DEFAULT for _ in range(4)]
    ...
    def prep_round(self):
        ...
        for p in range(4):
            for i in range(13):
                ...
                self.closed_hand_counts[p][tile.to_int()] += 1
                if tile.is_red5():
                    self.red5_hidden[p][tile.to_int() // 9] = 0
                    self.red5_closed_hand[p][tile.to_int() // 9] = 1
        ...
        self.visible_dora[dora_indicator.to_int()] += 1
        self.dora_revealed_no += 1
        for p in range(4):
            self.hidden_tile_counts[p][dora_indicator.to_int()] -= 1
            if dora_indicator.is_red5():
                self.red5_hidden[p][dora_indicator.to_int() // 9] = 0
    ...
    def update_board(self, play_sound_name="", update_player_id=False):
        # update board
        if not self.board or not self.gui.playing:
            exit()
        if update_player_id:
    ...
    def rest_of_the_logic_to_rewrite(self):
        # GAME LOGIC
